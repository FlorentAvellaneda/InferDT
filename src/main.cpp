...
void replace(std::vector<int> &tree, unsigned long a, unsigned long b) {
    std::deque<tuple<unsigned long, unsigned long> > toReplace = {make_tuple(a, b)};
    ...
    while (toReplace.size()) {
        ...
        if (b < tree.size()) {
            tree[a] = tree[b];
            toReplace.push_back(make_tuple(a * 2, b * 2));
            toReplace.push_back(make_tuple(a * 2 + 1, b * 2 + 1));
        } else {
...
bool verifierAppartient(std::vector<int> &inferedTree, std::vector<int> &e, int idClass) {
    std::vector<std::pair<unsigned int, std::vector<int>>> pos = {
            std::pair(1, std::vector<int>(e.size()))
    };
    while (pos.size()) {
        if (inferedTree[pos[0].first] < 0) {
            if (-inferedTree[pos[0].first] - 1 != idClass) {
                return false;
            }
            pos[0] = pos.back();
            pos.pop_back();
...
bool verifier(unsigned int nbFeature, std::vector<int> &generator, std::vector<int> &inferedTree, unsigned int pos = 1, std::vector<int> element = std::vector<int>()) {

    if (element.size() == 0)
        element.resize(nbFeature);

    if (generator[pos] < 0) {
        int idClass = -generator[pos] - 1;
        return verifierAppartient(inferedTree, element, idClass);
    }

    assert(generator[pos] >= 0);
    unsigned int feature = static_cast<unsigned int>(generator[pos]);

    if (element[feature] == 1)
        return verifier(nbFeature, generator, inferedTree, pos * 2 + 1, element);
    if (element[feature] == -1)
        return verifier(nbFeature, generator, inferedTree, pos * 2, element);

    element[feature] = 1;
    if (!verifier(nbFeature, generator, inferedTree, pos * 2 + 1, element))
        return false;
...
public:
    ...
    void addConstraints_Feature(const vector<bool> &newElement, unsigned int nb, Minisat::vec<Minisat::Lit> &clause, unsigned int q, unsigned int lvl) {
...
tuple<vector<bool>, unsigned int> distinguish(const std::vector<int> &tree, const vector<vector<vector<bool> > > &S) {
...
std::vector<int> inferTreeReduceNode(unsigned int k, const vector<vector<vector<bool> > > &S, bool verbose = false) {
    ...
    while (infer.inferModel()) {
        ...
        unsigned int label;
        std::tie(element, label) = distinguish(tree, S);
...
std::vector<int> inferTree(unsigned int k, const vector<vector<vector<bool> > > &S, int maxNumberOfStates = -1) {
    std::vector<int> tree;
    unsigned int nbFeature = 0;
    ...
    if (maxNumberOfStates != -1)
        infer.addConstraints_MaxLeaves(maxNumberOfStates / 2 + 1);
 ...
 */
unsigned int
updateSetOfType(const string &link, vector<set<string> > &elementString, vector<set<double> > &elementDouble,
    ...
    while (csv.next()) {
        ...
        if (tmp.size() != nbFeature + 1) {
            std::cerr << "CSV format error: " << (nbFeature + 1) << " columns inspected in " << link << " line " << line
                      << "." << std::endl;
            exit(-1);
        }
 ...
 */
std::tuple<vector<map<string, vector<bool> > >, vector<map<double, vector<bool> > > >
convertTypeToBin(const vector<set<string> > &elementString, const vector<set<double> > &elementDouble) {
 ...
 */
vector<vector<vector<bool> > > LoadAsBinaryFeature(const string& link) {
...
vector<vector<vector<vector<bool> > > > FinalLoadAsBinaryFeatureCross(string link, unsigned int nombre) {
    ...
    for (unsigned int c = 0; c < all.size(); c++) {
        for (auto features: all[c]) {
            all2.emplace_back(c, features);
        }
    }
    std::shuffle(all2.begin(), all2.end(), std::mt19937(std::random_device()()));
 ...
 */
std::tuple<vector<vector<vector<bool> > >, vector<vector<vector<bool> > > >
LoadAsBinaryFeature(const string& link_train, const string& link_test) {
...
int main(int argc, char *argv[]) {
    ...
    string testset = "";
    auto inferCmd = app.add_subcommand("infer",
                                       "infer an optimal decision tree (\"xdot\" required for print the result)")->callback(
            [&file, &minimizeOnlyDepth, &testset]() {
    ...
    auto benchCmd = app.add_subcommand("bench", "Benchmark with cross-validation")->callback(
            [&file, &nbfold, &minimizeOnlyDepth]() {
                ...
                for (int i = 0;; i++) {
                    ...
                    std::cout << endl << endl << "===== RESULT AFTER " << (i + 1) << " " << nbfold
